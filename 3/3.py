...
table = [['0'] * m for i in range(n)] # init table
startPoint = [int(n/2) -1, int(m/2) -1] # start point in the middle
table[startPoint[0]][startPoint[1]] = '1' # start point init

def fillTable(arg):
    '''Fill the table with 1 according to
    the formula '''
    x = 0
    y = 0
...
def searchIntersections():
    '''# x is when all adiacent points are 1
    # 00100
    # 01110
    # 00100
    # so it need to be 5 of ones in this sequence'''
    pointsToReturn = []
    for row in range(n):
        if (row != 0 and row != n):
            for col in range(n):
